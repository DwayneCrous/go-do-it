	...
	case tea.KeyMsg:
		...
		case modeView:
			...
			case "a":
				   ...
				   return m, m.textInput.Focus() // Ensure text input receives focus and input
			case "d":
				if len(m.todos) > 0 {
		...
		case modeAdd:
			var cmd tea.Cmd
			m.textInput, cmd = m.textInput.Update(msg) // Pass key events to text input
			...
			return m, cmd // Return early so no other keys are processed here
...
func (m model) View() string {
	...
	b.WriteString("\n\n")
	b.WriteString("Controls: j/down k/up a:add d:delete r:reload q:quit\n")
